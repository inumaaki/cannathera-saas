...
/* Branded A4 report — mirrors the client's "Muster-Monatsreport" (concept PDF
   ...
   satisfaction, appointment prep, Cannathera summary. Watermark + the mandatory
   "Powered by Cannathera" mark; practice logo added when co-branded. */
...
/** Start → end bar for one metric, with the % change badge. */
function metricRow(m: ReportData['metrics'][number]): string {
  ...
  const pct = (v: number) => Math.min(100, (v / 10) * 100);
  return `<tr>
...
export function reportHtml(d: ReportData, logoDataUri: string | null): string {
...
/** Running footer for every page — carries the mandatory co-branding mark. */
export function footerTemplate(d: ReportData): string {
  const period = `${de(d.periodStart)} – ${de(d.periodEnd)}`;
...
/** Renders the report HTML to a PDF buffer via headless Chromium. */
export async function renderReportPdf(d: ReportData): Promise<Buffer> {
  const puppeteer = await import('puppeteer');
  const fs = await import('fs/promises');
  const path = await import('path');

  // Embed the practice logo (co-branding) or the Cannathera mark as the watermark.
