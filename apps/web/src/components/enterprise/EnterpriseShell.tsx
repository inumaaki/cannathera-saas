...
const NAV = [
  { href: "/enterprise", key: "overview", icon: "dashboard" },
  { href: "/enterprise/partners", key: "partners", icon: "hub" },
  { href: "/enterprise/integrations", key: "integrations", icon: "api" },
  { href: "/enterprise/branding", key: "branding", icon: "palette" },
  { href: "/enterprise/billing", key: "billing", icon: "receipt_long" },
  { href: "/enterprise/reports", key: "reports", icon: "bar_chart" },
  { href: "/enterprise/team", key: "team", icon: "groups" },
  { href: "/enterprise/settings", key: "settings", icon: "settings" },
] as const;

export type Notice = { id: string; icon: string; text: string; href: string };

/* Enterprise Portal frame (Figma 8.x). The Figma sidebar is labelled "Pharmacy
   Portal" on every enterprise screen — a copy/paste slip in the design, corrected
   here to "Enterprise Portal". */
export function EnterpriseShell({
  ...
  const t = useTranslations("enterprise.shell");
  ...
  function handleSearch(e: React.FormEvent<HTMLFormElement>) {
    ...
    router.push(
      q ? { pathname: "/enterprise/partners", query: { q } } : "/enterprise/partners",
    );
  }
    ...
    <div className="flex min-h-dvh bg-surface">
      <aside className="sticky top-0 flex h-dvh w-64 shrink-0 flex-col bg-brand text-white max-lg:w-16 print:hidden">
        ...
        <div className="mx-4 mt-6 rounded-xl bg-white/10 px-4 py-3 max-lg:hidden">
          ...
          <p className="mt-0.5 truncate font-display text-lg font-bold text-white">
            {networkName}
          </p>
        </div>

        <nav className="mt-6 flex flex-1 flex-col gap-1 px-3" aria-label={t("portal")}>
                        ...
                        {notices.map((n) => (
                          <li key={n.id}>
                            ...
                            >
                              <span
                                aria-hidden
                                className="msym text-[20px] text-accent-print"
                              >
                                {n.icon}
                              </span>
