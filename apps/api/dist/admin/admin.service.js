...
let AdminService = class AdminService {
    ...
    async listPartners() {
        return this.prisma.organization.findMany({
            where: {
                type: {
                    in: [client_1.OrgType.PRACTICE, client_1.OrgType.PHARMACY, client_1.OrgType.ENTERPRISE, client_1.OrgType.CLINIC],
                },
            },
            include: {
                settings: true,
    ...
    async togglePartner(orgId) {
        ...
        if (activeSub) {
            ...
            const plan = await this.prisma.pricingPlan.findFirst({
                where: { tier: client_1.SubscriptionTier.PREMIUM },
            });
    ...
    async onboardPartner(dto) {
        ...
        let role = client_1.Role.DOCTOR;
        if (dto.type === client_1.OrgType.PHARMACY)
            role = client_1.Role.PHARMACY;
        if (dto.type === client_1.OrgType.ENTERPRISE)
        ...
        if (!plan) {
            plan = await this.prisma.pricingPlan.create({
                data: {
                    ...
                    monthlyPrice: dto.planTier === client_1.SubscriptionTier.PREMIUM ? 349 : 149,
                    reviewCap: 100,
        ...
        const tempPassword = (0, crypto_1.randomBytes)(4).toString('hex');
    ...
    async toggleUser(userId) {
        ...
        if (PROTECTED_EMAILS.includes(user.email)) {
            throw new common_1.BadRequestException('CANNOT_SUSPEND_ADMIN_ACCOUNT');
        }
    ...
    async updatePricingPlan(id, dto) {
        ...
        if (!plan)
            throw new common_1.NotFoundException('PRICING_PLAN_NOT_FOUND');
