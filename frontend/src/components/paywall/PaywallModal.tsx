...
export function PaywallModal({ isOpen, onClose, type }: Readonly<PaywallModalProps>) {
  ...
  async function handleCheckout(planTier: string) {
    ...
    try {
      ...
      alert("Billing server unreachable.");
    } finally {
      setBusy(false);
    }
  }
    ...
    >
      ...
      >
        ...
        <p className="mt-2 text-center text-sm text-muted">
          {isPatient
            ? "Unlock clinical PDF exports, deep-dive therapy insights, and premium features."
            : "Activate your professional clinic/pharmacy package to compile audits and complete reviews."}
        </p>
        ...
        <ul className="mt-5 space-y-2.5 rounded-xl bg-surface p-4 text-sm text-ink-strong">
          <li className="flex items-center gap-2.5">
            ...
            <span>{isPatient ? "Export legally-compliant PDF reports" : "Complete structured monthly reviews"}</span>
          </li>
          <li className="flex items-center gap-2.5">
            <span className="msym text-[18px] text-brand">check_circle</span>
            <span>{isPatient ? "Traceable 90-day progress indexes" : "Compile compliance audit logs"}</span>
          </li>
          <li className="flex items-center gap-2.5">
            <span className="msym text-[18px] text-brand">check_circle</span>
            <span>{isPatient ? "Priority clinic support channels" : "Dedicated partner success manager"}</span>
          </li>
        </ul>
          ...
          {isPatient ? (
            <>
              ...
              >
                ...
                <span className="text-brand">€9.99 / Month</span>
              </button>
              <button
                type="button"
                disabled={busy}
              ...
              >
                <span>PLUS Plan</span>
                <span className="text-brand">€29.99 / Month</span>
              </button>
              <button
                type="button"
                disabled={busy}
            ...
            <>
              <button
                ...
                className="flex w-full items-center justify-between rounded-xl border border-hairline bg-white px-4 py-3.5 text-sm font-bold text-ink-strong hover:border-brand transition-colors disabled:opacity-50"
              >
                <span>FLEX Partner Plan</span>
                <span className="text-brand">€449.00 / Month</span>
              </button>
              <button
                type="button"
                disabled={busy}
                ...
                className="flex w-full items-center justify-between rounded-xl border border-hairline bg-white px-4 py-3.5 text-sm font-bold text-ink-strong hover:border-brand transition-colors disabled:opacity-50"
              >
                <span>FLASHBACK S Plan</span>
                <span className="text-brand">€899.00 / Month</span>
              </button>
              <button
                type="button"
                disabled={busy}
                ...
                className="flex w-full items-center justify-between rounded-xl bg-gradient-to-tr from-brand to-pine px-4 py-3.5 text-sm font-bold text-white hover:opacity-90 shadow-md transition-opacity disabled:opacity-50"
              >
                <span>FLASHBACK M Plan</span>
                <span>€1,599.00 / Month</span>
              </button>
            </>
          )}
        </div>
        ...
        <div className="mt-5 flex flex-col items-center justify-center gap-2 border-t border-hairline pt-4 text-xs text-muted">
          ...
          <p className="text-[10px]">Secure, 256-bit encrypted billing powered by Stripe.</p>
        </div>
      </div>
    </div>
  );
